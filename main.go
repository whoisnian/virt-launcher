...
func main() {
	...
	oriImagePath := cache.ImagesDir(img.BaseName())
	...
	finalImagePath := cache.BootDir(finalImageName)
	...
	cloudIsoCacheDir := cache.CloudInitDir(fmt.Sprintf("%s.%s", global.CFG.Name, timeStr))
	cloudIsoName := fmt.Sprintf("%s.%s.iso", global.CFG.Name, timeStr)
	cloudIsoPath := cache.BootDir(cloudIsoName)
